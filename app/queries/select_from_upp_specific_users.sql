...
SELECT
    ...
    ,t24._Description						AS enforcement_proceedings
FROM
	_AccumRg16554 t1
    ...
    LEFT JOIN transport_managers as t22 on
        ...
        AND t22.rn = 1
    LEFT JOIN _InfoRg14396 as t23 on
        t3._IDRRef = t23._Fld14397_RRRef
        AND convert(nvarchar(36), t23._Fld14398RRef, 2) = 'BAAFA4BF012C04CD11EF085A2923BB08'
    LEFT JOIN _Reference66 t24 on
     	t23._Fld14399_RRRef = t24._IDRRef
WHERE
...
SELECT DISTINCT
    ...
    ,CONVERT(VARCHAR(255),  '"'+REPLACE("enforcement_proceedings", '"', '""')+'"')			    AS "enforcement_proceedings"
FROM
    ct3
WHERE
    ...
    AND COALESCE(
        "Ответственный Привязка КодОбмена"
        ,"Ответственный тран КодОбмена"
        ,"Ответственный ДЗКЗ КодОбмена"
        ,"Ответственный ЗРС КодОбмена"
        ,"Ответственный док КодОбмена") IN ($(upp_users_codes))
ORDER BY
